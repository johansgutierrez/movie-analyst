...
dir=$(ls /opt/app | grep "movie-analyst-ui")
if ["$dir"== "movie-analyst-ui"]; then
echo" Ya existe la carpeta movie-analyst-ui "
else
mkdir /opt/app
cd /opt/app
git clone https://github.com/juan-ruiz/movie-analyst-ui.git
cd movie-analyst-ui
sed -i 's/localhost/192.168.31.222/g' server.js
...
cd /opt/app/scripts
touch server.sh
echo "PORT=3030 node /opt/app/movie-analyst-ui/server.js" >> /opt/app/scripts/server.sh
